...
| Build 4F.1C-A solicitud real desde invitación | Preparado | SQL versionado nuevo en `sql/29_create_job_request_from_invitation_rpc.sql`, pendiente de revisar y ejecutar; `sql/05_grants.sql` añade el grant execute a authenticated; API `createJobRequestFromInvitation()` añadida en `src/lib/api/jobInvitations.ts`; la RPC crea `job_request` `pending` y marca la invitación como `accepted` en una sola operación transaccional; no abre chat, no revela dirección exacta, no toca Stripe/chat/pagos/acuerdos y el cliente sigue siendo quien acepta la solicitud; la UI queda pendiente para 4F.1C-B; sin SQL remoto ejecutado. |
...
| `create_job_request_from_invitation` | `sql/29_create_job_request_from_invitation_rpc.sql` | RPC versionada en repo para que un profesional approved responda una invitación creando una `job_request` `pending` y marcando la invitación como `accepted` en la misma transacción; pendiente de revisar y ejecutar en Supabase; `SECURITY DEFINER`, `search_path = public, pg_temp`, sin execute para `anon` ni `public`, y con grant previsto para `authenticated` en `sql/05_grants.sql`. |
...
- 4F.1C-A añade SQL versionado nuevo en `sql/29_create_job_request_from_invitation_rpc.sql`, pero todavía no se ha ejecutado en Supabase; también añade el grant previsto en `sql/05_grants.sql` y la API frontend `createJobRequestFromInvitation()` sin usar `service_role`.
...
- 4F.1C-A prepara la operación transaccional para responder una invitación enviando solicitud: crea `job_request` `pending`, marca la invitación como `accepted`, no abre chat, no revela dirección exacta, no toca pagos ni acuerdos y el cliente sigue siendo quien acepta la solicitud; la UI queda pendiente para 4F.1C-B.
