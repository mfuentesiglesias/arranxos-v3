...
import {
  ...
  type ApiAdminEconomySummary,
} from "@/lib/api/adminEconomy";
...
export default function AdminEconomiaPage() {
  ...
  const [realSummary, setRealSummary] = useState<ApiAdminEconomySummary | null>(null);
  const [realSummaryLoading, setRealSummaryLoading] = useState(false);
  const [realSummaryError, setRealSummaryError] = useState<string | null>(null);
  ...
  const loadRealSummary = useCallback(async () => {
    if (!isSupabase) {
      return;
    }

    setRealSummaryLoading(true);
    setRealSummaryError(null);

    try {
      const nextSummary = await getAdminEconomySummary();
      setRealSummary(nextSummary);
    } catch (error) {
      setRealSummary(null);
      setRealSummaryError(
        error instanceof Error && error.message
          ? error.message
          : "No pudimos cargar el resumen económico real.",
      );
    } finally {
      setRealSummaryLoading(false);
    }
  }, [isSupabase]);
  ...
  useEffect(() => {
    ...
    void loadRealSummary();
  }, [isSupabase, loadRealSummary]);
  ...
  return (
    <div className="flex-1 flex flex-col bg-sand-50">
      <StatusBar />
      <TopBar title="Economía" subtitle={isSupabase ? "Resumen lógico Supabase" : "Visión central del flujo económico mock"} />
      <ScreenBody className="px-4 pt-3 pb-6">
        <div data-testid="admin-economy-page" className="flex flex-col gap-3">
          ...
          {isSupabase ? (
            <>
              {realSummaryLoading && (
                <Card className="text-[12px] text-ink-600 leading-snug">
                  Estamos cargando el resumen económico lógico real.
                </Card>
              )}
              ...
              {realSummary && (
                <Card className="bg-ink-900 text-white border-ink-900" testId="admin-economy-summary">
                  <div className="grid grid-cols-2 gap-3 text-[12px]">
                    <SummaryMetric label="Agreements pending" value={String(realSummary.agreementsPending)} />
                    <SummaryMetric label="Agreements protected" value={String(realSummary.agreementsProtected)} />
                    <SummaryMetric label="Agreements released" value={String(realSummary.agreementsReleased)} />
                    <SummaryMetric label="Agreements refunded" value={String(realSummary.agreementsRefunded)} />
                    <SummaryMetric label="Pend. confirmación" value={String(realSummary.jobsCompletedPendingConfirmation)} />
                    <SummaryMetric label="Jobs completed" value={String(realSummary.jobsCompleted)} />
                    <SummaryMetric label="Importe protegido lógico" value={formatEuro(realSummary.protectedGrossAmount)} />
                    <SummaryMetric label="Importe liberado lógico" value={formatEuro(realSummary.releasedGrossAmount)} />
                    <SummaryMetric label="Importe reembolsado lógico" value={formatEuro(realSummary.refundedGrossAmount)} />
                    <SummaryMetric label="Comisión estimada protegida" value={formatEuro(realSummary.protectedEstimatedCommission)} />
                    <SummaryMetric label="Comisión estimada liberada" value={formatEuro(realSummary.releasedEstimatedCommission)} />
                    <SummaryMetric label="Agreements con paid_at" value={String(realSummary.agreementsPaidCount)} />
                  </div>
                  <div className="mt-3 rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-[11px] leading-snug text-white/85">
                    Resumen lógico Supabase · sin Stripe real. Los `payment_status` representan el estado interno del acuerdo, no un cobro real en PSP.
                  </div>
                  <div className="mt-3 text-[11px] text-white/75 leading-snug">
                    Jobs: agreed {realSummary.jobsAgreed} · escrow funded {realSummary.jobsEscrowFunded} · dispute {realSummary.jobsDispute} · cancelled {realSummary.jobsCancelled} · agreements cancelled {realSummary.agreementsCancelled} · agreements con released_at {realSummary.agreementsReleasedCount}
                  </div>
              ...
              <Card className="border-sand-200/70 bg-white text-[12px] text-ink-600 leading-snug">
                El resumen superior y el auto-release manual ya usan datos reales de Supabase. El desglose fila a fila por trabajo sigue pendiente para evitar mezclar nombres y cálculos demo con estados reales.
              </Card>
            </>
          ) : (
            <>
