...
import { listMyClientChats, type ApiClientChatThread } from "@/lib/api/chat";
...
function getJobStatusLabel(status: ApiClientChatThread["jobStatus"]): string {
  switch (status) {
    case "in_progress":
      return "En curso";
...
export default function ClientChatPage() {
  ...
  const [threads, setThreads] = useState<ApiClientChatThread[]>([]);
      ...
      try {
        ...
        if (!profile || profile.role !== "client") {
          if (!cancelled) {
            setThreads([]);
            setLoading(false);
            setError("Solo un cliente puede acceder a chats reales.");
          }
          return;
        }

        const nextThreads = await listMyClientChats();
  ...
  return (
    <div className="flex-1 flex flex-col" data-testid="client-chat-page">
      <StatusBar />
      <div className="bg-white border-b border-sand-200/70 px-5 pt-2 pb-3">
        ...
        <div className="mt-1 text-[12px] text-ink-500 leading-snug">
          Los chats se abren cuando aceptas una solicitud de un profesional.
        </div>
      </div>

      <ScreenBody className="px-4 pt-4 pb-6">
        {loading ? (
          <Card className="bg-white border-sand-200/70" testId="client-chat-loading">
            <div className="py-6 text-center text-[12px] text-ink-500">Cargando chats reales…</div>
          </Card>
        ) : error ? (
          <Card className="bg-rose-50 border-rose-100" testId="client-chat-error">
          ...
          <Card className="bg-white border-sand-200/70" testId="client-chat-empty">
            <div className="py-6 text-center text-[12px] text-ink-500 leading-snug">
              Todavía no tienes chats abiertos en tu cuenta de cliente.
            </div>
          </Card>
        ) : (
          <div className="flex flex-col gap-2.5 pb-20" data-testid="client-chat-list">
            {threads.map((thread) => (
              <Card key={thread.chatId} className="bg-white border-sand-200/70" testId="client-chat-card">
